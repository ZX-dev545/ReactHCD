...
import RadiusSvg from '../assets/svg/game/radius.svg';


export default function Radius({ onComplete }: { onComplete: () => void }) {
  const [keyboardVisible, setKeyboardVisible] = useState(false);
  const [input, setInput] = useState('');
  const [result, setResult] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const answer = "Kyoto Garden, London, UK";
  ...
  return (
    <>
      ...
      <Text style={{
        ...
        within the circle shown below
      </Text>

      <View style={{ position: 'absolute', top: 100, width: '100%', zIndex: 0 }}>
        <RadiusSvg width='100%' preserveAspectRatio='MidXMidY' />
      </View>
